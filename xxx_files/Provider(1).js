...
__sco.management.main = function () {
                ...
                if (typeof obj['func'] !== "undefined" && typeof obj['args'] !== "undefined") {
                    ...
                    switch (func) {
                        ...
                        case "get":
                            ...
                            break;
                        case "cookieget":
                            store.cookieget.call(window, { 'ticket': ticket, 'args': args, 'classic': __sco.type(__sco.tonumber(obj['ticket'])) });
                            break;
                        case "remove":
    ...
    function validate(obj) {
        ...
        if (scobj.referrers.indexOf(obj.origin) > -1 && search(__sco.tryparse(obj.data).guid, scobj.guid))
            return true;
        else
            return false;
    }
...
__sco.storage = function () {
    ...
    this.get = function (obj) {
        ...
        if (!!data && __sco.type(scobj.RelatedClientIds) === "array") {
            data = checkRelatedClients(data);
        }
    ...
    /** Regardless of cookie setting ability, provide a get functionality **/
    this.cookieget = function (obj) {
        obj.args[0] += scobj.guid;
        var data = cookieget(obj.args[0], obj.args.length > 1 ? obj.args[1] : false);
        if ((__sco.type(obj.ticket) === "number" && obj.ticket >= 0) || obj.classic === "boolean")
            __sco.management.callback(obj.classic === "boolean" ? data : { 'data': data, 'ticket': obj.ticket });
    }
    ...
    /**         PRIVATE FUNCTIONS           **/
    /*****************************************/
    function checkRelatedClients(d) {
        function isNotEmpty(a) {
            return typeof a === "string" && a !== "";
        }

        __sco.each(scobj.RelatedClientIds, function (ix, val) {
            var thisLocalToCheck = "__sc" + val.ClientId + "" + val.ApiKey, rs = null;
            if (__sco.type((rs = localget(thisLocalToCheck))) === "object") {
                if (__sco.type(rs.c) === "object") {
                    __sco.each(d.c, function (i, v) {
                        if (__sco.type(d.c[i]) === "object") {
                            __sco.each(d.c[i], function (o, w) {
                                if (isNotEmpty(d.c[i][o]) === false && isNotEmpty(rs.c[i][o])) {
                                    d.c[i][o] = rs.c[i][o];
                                }
                            });
                        } else {
                            if (isNotEmpty(d.c[i]) === false && isNotEmpty(rs.c[i])) {
                                d.c[i] = rs.c[i];
                            }
                        }
                    });
